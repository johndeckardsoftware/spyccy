...
/** Set chip type. */
int ayemu_set_chip_type(ayemu_chip_t type, int *custom_table)
{
if (!check_magic(&ay))
		return 0;
...
/** Set chip frequency. */
void ayemu_set_chip_freq(int chipfreq)
{
  if (!check_magic(&ay)) return;
 ...
 */
int ayemu_set_sound_format(int freq, int chans, int bits)
{
  if (!check_magic(&ay))
    return 0;
 ...
 */
int ayemu_set_stereo(ayemu_stereo_t stereo_type, int *custom_eq)
{
  int i;
  int chip;
 ...
 */
void ayemu_set_regs(ayemu_ay_reg_frame_t regs)
{
  if (!check_magic(&ay)) return;
...
static void prepare_generation(ayemu_ay_t *ay)
{
  ...
  if (ay->default_chip_flag) ayemu_set_chip_type(AYEMU_AY, NULL);

  if (ay->default_stereo_flag) ayemu_set_stereo(AYEMU_ABC, NULL);

  if (ay->default_sound_format_flag) ayemu_set_sound_format(44100, 2, 16);
 ...
 */
void *ayemu_gen_sound(void *buff, size_t sound_bufsize)
{
  int mix_l, mix_r;
  int tmpvol;
  int m;
