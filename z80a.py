...
#z80profile2.init_trace(log='./tmp/z80a5.log', profile=True, pcr_enabled=True, pcr_start=0x8000)
...
def runUntil(max_t_state):
    global t, t_op, i, r, pc, sp, iff1, iff2, im, interruptible, halted, opcode_prefix
    global uSource, betadisk, betadisk_active
    global tapedeck, b
    #global tape_traps_enabled, will_trap
    ...
    while t < max_t_state:
        ...
        #  machine.tapedeck.fast_save(sys.modules[__name__])

        #if (pc == 0x04c2) and tape_traps_enabled and will_trap:
        #  machine.tapedeck.basic_save(sys.modules[__name__])       # tape saveing trap

        if (pc == 0x056b and tapedeck and not tapedeck.is_tape_empty and not tapedeck.is_opening
            and not tapedeck.is_tape_playing and not tapedeck.is_manually_paused):
                machine.tapedeck.play_tape(True)

        # if a tape is playing, process the tape data.
        if tapedeck.is_tape_playing:
            # when the rom tape loading routine has started (checked at ld_edge_1 / ld_edge_2 and ld_bytes),
            # increase emulation speed if speed load is enabled.
            if (pc == 0x05e8 or pc == 0x05e4 or pc == 0x056b) and tapedeck.tape_speed_load and not tapedeck.is_speed_loading:
                tapedeck.is_speed_loading = True
                machine.constrain_50_fps = False
                machine.beeper.mute(1)
                machine.AY.mute(1)

            # if edge detection is enabled, try to shortcut the rom tape loading routines.
            #if tapedeck.tape_edge_detect:
            #    # if edge detection fails, just proceed with the standard tape loading process.
            #    if not tapedeck.edge_detection(sys.modules[__name__]):
            #        tapedeck.process_tape_data(t_op)
            #else:
            #    tapedeck.process_tape_data(t_op)
            tapedeck.process_tape_data(t_op)

            # flash load tape blocks by bypassing the ld_bytes routine
            # and directly store the block data in memory at the start position,
            # fetched from the ix register pair
            if pc == 0x056b and tapedeck.tape_fast_load:
                tape_index = tapedeck.fast_load(sys.modules[__name__])
                if tape_index >= 0:
                    tapedeck._last_read_tape_index = tape_index
...
# fake z80 opcode to call python code >>>>>
def opcode_ed_0():
    # reg a contains the number of the function to be executed
    meta_opcode.execute(sys.modules[__name__])
# <<<<<

# fake z80 opcode to override ROM routines >>>>>
...
def opcode_ed_2():  #  basic_save
    ...
    machine.tapedeck.basic_save(sys.modules[__name__])
